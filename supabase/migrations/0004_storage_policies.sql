...
CREATE POLICY "Leitura por organização — documentos"
  ...
  USING (
    ...
    AND (storage.foldername(name))[1] = auth.organizacao_id()::text
  );
...
CREATE POLICY "MFA obrigatório para gestor e coord_comite — storage"
  ...
  FOR ALL
  TO authenticated
  USING (auth.papel() NOT IN ('gestor', 'coord_comite') OR (auth.jwt() ->> 'aal') = 'aal2')
  WITH CHECK (auth.papel() NOT IN ('gestor', 'coord_comite') OR (auth.jwt() ->> 'aal') = 'aal2');
