...
/**
 * Fase 2, item 7: marcadores {{nome}}, {{cpf}}, {{endereco}}, {{objeto}}, {{valor}},
...
describe("substituirMarcadores", () => {
  it("substitui todos os 8 marcadores suportados", () => {
    const corpo =
      "{{nome}}, CPF {{cpf}}, residente em {{endereco}}. Objeto: {{objeto}}. " +
    ...
    expect(resultado).toBe(
      "Ana Clara Fagundes, CPF 529.982.247-25, residente em Rua das Flores, 123. " +
        "Objeto: Militância e Mobilização de Rua. Valor: R$ 1.500,00 (um mil e quinhentos reais). " +
        "Vigência: 01/09/2026 a 03/10/2026.",
    );
  });
  ...
  it("nunca deixa um marcador sem substituir quando todos os dados estão presentes", () => {
    const corpo = "{{nome}} {{cpf}} {{endereco}} {{objeto}} {{valor}} {{valor_extenso}} {{vigencia_inicio}} {{vigencia_fim}}";
    const resultado = substituirMarcadores(corpo, dados);
