...
import { valorExtenso } from "@/lib/contratos/valor-extenso";
...
describe("valorExtenso", () => {
  it("3.553,00 → três mil quinhentos e cinquenta e três reais", () => {
    expect(valorExtenso(3553)).toBe("três mil quinhentos e cinquenta e três reais");
  });

  it("4.353,00 → quatro mil trezentos e cinquenta e três reais", () => {
    expect(valorExtenso(4353)).toBe("quatro mil trezentos e cinquenta e três reais");
  });

  it("2.200,00 → dois mil e duzentos reais", () => {
    expect(valorExtenso(2200)).toBe("dois mil e duzentos reais");
  });

  it("1.500,00 → mil e quinhentos reais", () => {
    expect(valorExtenso(1500)).toBe("mil e quinhentos reais");
  });

  it("1.000.000,00 → um milhão de reais", () => {
    expect(valorExtenso(1000000)).toBe("um milhão de reais");
  });
  ...
  it("aceita valor numeric do Drizzle como string", () => {
    expect(valorExtenso("3553.00")).toBe("três mil quinhentos e cinquenta e três reais");
  });
});
